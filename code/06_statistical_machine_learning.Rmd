...
```{r}
dist <- attr(knn_pred, "nn.dist") %>% as.vector()
...
```{r}
graph <- xgb_tbl %>% 
    ggplot(aes(borrower_score, payment_inc_ratio, color = pred_default, shape = pred_default, size = pred_default)) +
    geom_point(alpha = 0.8) +
...
result <- cbind(params, avg_error) %>% as_tibble()

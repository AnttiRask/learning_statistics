...
p +
    ...
    scale_y_continuous(expand = c(0, 0), limits = (c(300, 450))) +
...
```{r}
prop_type_dummies <- model.matrix(~ PropertyType -1, data = house) %>% as_tibble()
...
house_98105_augmented %>%
    ...
    scale_size(range = c(1, 20 * sqrt(max(house_98105_augmented$cooks_D)))) +
    scale_x_continuous(
